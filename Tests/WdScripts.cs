  ...
  public void Run(DBConnections2.dbconn _db,DataRow dr, Construction model)
  {
   ...
   try
   {
    ...
    if( inRange( drOI.idproductiontype, 105, 1191, 1551, 1891, 2186 ) ) //Москитная сетка
    {
     ...
     bool antiCat = false; bool innerMS = false;
     string strMS = """""""";
     if( model != null )
     {
     ...
     switch(drOI.idconstructiontype)
     {
      case 8:
       if( !antiCat )
       {
        ...
        Order.DocWork.AddWorkOperForItem(""""Оконная, шт."""",1);
       ...
       else
       {
        ...
        Order.DocWork.AddWorkOperForItem(""""Антикошка, шт."""",1);
      ...
      case 10:
       if( !antiCat )
       {
        ...
        Order.DocWork.AddWorkOperForItem(""""Дверная, шт."""",1);
       }
       else
       {
        f = drOI.AddFinparam( 287, 4, 4 ); // Вид москитной сетки
     ...
     if( model.MoskitList.Count > 0 )
     {
      ...
      if( inRange( typeMS2, """"Оконная"""", """"Оконная Стандарт"""", """"Оконная усиленная"""", """"Усиленная Стандарт"""" ) && model.Height > 1430 && !model.ImpostList.Any( x=>( x.AngNakl == 0 || x.AngNakl == 180 ) ) )
      {
       // и287
      ...
      {
       // MessageBox.Show( """"A="""" + typeMS2 + """", cnt="""" + model.ImpostList.FindAll( x=>( x.AngNakl == 0 || x.AngNakl == 180 ) ).Count );
      }
