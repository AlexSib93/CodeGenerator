...
";
        ...
        private string GetComponentsText()
        {
            return (FormInfo.Components!= null)
                ? string.Join(Environment.NewLine, FormInfo.Components.Select(c => TsPropBuilder.GetTsComponent(c, "items={items} onEdit={setItem} onDelete={handleDelete} onAdd={addItem} "+((c.Type == ComponentTypeEnum.Grid) ? " enableFilters={true}" : ""))))
                : "";
        }
...
{(EditForm != null ? $@"import {EditForm.Name} from ""./{EditForm.Name}"";" : "")}";
        ...
        private string ImportEnumTypes()
        {
            var enumLookUpTypes = FormInfo.Components
                .Where(c => c.Type == ComponentTypeEnum.Grid)
